...
| --- | --- | --- | --- |
| New frontend | `pnpm --dir frontend dev` (`:3001`) | `:3002` | active |
...
- [x] Every nav item in `frontend/src/components/shell/nav-config.ts`
      has a corresponding real implementation in `frontend/src/routes/`.
...
- [x] `frontend/Dockerfile` produces a runnable image
...
5. **Update operator-facing references.**
   ...
   - [`../AGENTS.md`](../AGENTS.md) "Where things live" table — change
     the `webui/` entry to `frontend/` and add a `webui-legacy/` row.
...
3. File a bug, restore the relevant stub in
   `frontend/src/routes/<route>/page.tsx` so the legacy app handles
