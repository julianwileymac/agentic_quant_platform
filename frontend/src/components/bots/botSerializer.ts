/**
 * Serializer that turns a node-and-wire `FlowGraph` (built with the Bot
 * Builder) into a server-side `BotSpec` payload, and the inverse for
 * loading an existing bot back into the canvas.
 *
 * Each node maps to a single spec slot:
 *
 ...
 *
 * Edges are informational on the canvas; order is taken from the node
 * array, which the FlowCanvas keeps stable across renders.
 */
...
export function serializeBotSpec(graph: FlowGraph, meta: BotSpecMeta): Record<string, unknown> {
  ...
  for (const node of graph.nodes) {
    ...
    switch (node.data.kind) {
      case "Universe": {
        const symbols = Array.isArray(params.symbols) ? (params.symbols as string[]) : [];
      ...
      case "Deploy":
        ...
        break;
      default:
        // Unknown nodes are ignored (annotation / sticky-note in future).
        break;
    }
  }
