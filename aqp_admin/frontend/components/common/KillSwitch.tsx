 ...
 * `<ConfirmFrictionDialog />` (the operator must type the confirm
 * phrase verbatim).
 *
 * Ported from `aqp_admin_ui/src/components/common/KillSwitch.tsx`.
 */
import { useState } from "react";

import { adminApi, type HaltAllResponse } from "@/lib/api/client";
...
export function KillSwitch() {
  ...
  async function fire(reason: string) {
    ...
    try {
      ...
      console.error("kill-switch fan-out failed: code=%s", code ?? "unknown");
    } finally {
      setBusy(false);
      setOpen(false);
    }
