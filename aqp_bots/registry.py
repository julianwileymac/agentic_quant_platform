...
- **YAML-driven**: ``aqp_bots/templates/<kind>/*.yaml`` is scanned on first
  lookup for any spec not already registered. ``configs/bots`` remains a
  compatibility fallback during the repository split. Discovered specs are
...
from aqp_bots.spec import BotSpec, load_specs_from_dir
...
_DEFAULT_DIR = _PACKAGE_DIR / "templates"
_LEGACY_DIR = _REPO_ROOT / "configs" / "bots"


def _default_dirs() -> tuple[Path, ...]:
    """Return template directories in scan order, preserving test monkeypatches."""
    dirs = [_DEFAULT_DIR]
    if _LEGACY_DIR != _DEFAULT_DIR:
...
def _ensure_yaml_scan() -> None:
    for directory in _default_dirs():
        ...
        if not directory.exists():
            continue
        for spec in load_specs_from_dir(str(directory)):
            with _LOCK:
...
def reload_yaml_dir(path: str | Path | None = None) -> int:
    ...
    """
    n = 0
    targets = (Path(path),) if path else _default_dirs()
