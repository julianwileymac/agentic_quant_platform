...
from aqp.rl.core.replay import BaseTrajectoryStore, InMemoryTrajectoryStore
...
class RLRuntime:
    ...
    def _snapshot_spec(self) -> tuple[str | None, str | None]:
        from aqp.rl.registry import persist_spec
    ...
    def _open_trajectory_store(self) -> None:
        ...
        if store_spec is None:
            try:
                from aqp.rl.trajectories.iceberg_writer import IcebergTrajectoryStore
...
def _summarise_history(rewards: list[float], history: list[float]) -> dict[str, float]:
    """Compact metrics dict mirroring :func:`aqp.rl.evaluator.summarise`."""
