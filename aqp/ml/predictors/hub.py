...
:mod:`aqp.ml.models` factories which already wrap XGBoost / sklearn /
...
from aqp.ml.predictors.spec import PredictorSpec
...
def _xgb_regression_factory(spec: PredictorSpec) -> Any:
    ...
    Delegates to :mod:`aqp.ml.models.tree` for the underlying XGBoost
    object (already a sanctioned registry entry). The hub adds the
    spec-aware hyperparam mapping.
    """
    try:
        from aqp.ml.models.tree import XGBModel
...
def _lstm_classification_factory(spec: PredictorSpec) -> Any:
    ...
    :class:`aqp.ml.models.torch.lstm.LSTMModel` shape and override the
