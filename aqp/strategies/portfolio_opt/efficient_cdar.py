"""Conditional Drawdown at Risk (CDaR) optimiser."""
...
STRATEGY_TAGS = ("portfolio", "risk", "cdar", "drawdown")


@portfolio("EfficientCDaRPortfolio", tags=STRATEGY_TAGS)
class EfficientCDaRPortfolio(IPortfolioConstructionModel):
    """CDaR-optimal allocation — tail drawdown control."""
    ...
    def __init__(
        ...
        beta: float = 0.95,
        objective: str = "min_cdar",  # or "efficient_return"
    ...
    def construct(self, signals: list[Signal], context: dict[str, Any]) -> list[PortfolioTarget]:
        ...
        if returns.empty or len(returns.columns) < 2:
            return _equal_weights(chosen, "CDaR fallback equal")
        weights = self._optimize(returns)
        if not weights:
            weights = _inverse_vol_weights(returns)
        return _targets_from_weights(weights, chosen, rationale_prefix="CDaR")
    ...
    def _optimize(self, returns: pd.DataFrame) -> dict[str, float]:
        try:
            ...
            logger.debug("pypfopt EfficientCDaR unavailable/failed", exc_info=True)
            return {}


__all__ = ["EfficientCDaRPortfolio"]
