"""CVaR-optimal portfolio construction.

Minimises the conditional value-at-risk (expected loss in the worst
``beta`` fraction of scenarios) at a given target return, or computes the
min-CVaR portfolio unconditionally. Delegates to
``pypfopt.efficient_frontier.EfficientCVaR`` when available; falls back to
the inverse-volatility allocation so paper-trading never breaks.
"""
...
STRATEGY_TAGS = ("portfolio", "risk", "cvar", "tail-risk")


@portfolio("EfficientCVaRPortfolio", tags=STRATEGY_TAGS)
class EfficientCVaRPortfolio(IPortfolioConstructionModel):
    """CVaR-optimal allocation (Rockafellar-Uryasev)."""
    ...
    def __init__(
        ...
        beta: float = 0.95,
        objective: str = "min_cvar",  # or "efficient_return"
    ...
    def construct(self, signals: list[Signal], context: dict[str, Any]) -> list[PortfolioTarget]:
        ...
        if returns.empty or len(returns.columns) < 2:
            return _equal_weights(chosen, "CVaR fallback equal")
        weights = self._optimize(returns)
        if not weights:
            weights = _inverse_vol_weights(returns)
        return _targets_from_weights(weights, chosen, rationale_prefix="CVaR")
    ...
    def _optimize(self, returns: pd.DataFrame) -> dict[str, float]:
        try:
            ...
            logger.debug("pypfopt EfficientCVaR unavailable/failed", exc_info=True)
            return {}


__all__ = ["EfficientCVaRPortfolio"]
