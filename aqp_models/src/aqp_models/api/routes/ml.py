...
def train_model(req: TrainRequest) -> TaskAccepted:
    ...
    from aqp_models.tasks.ml_tasks import train_ml_model
...
def run_experiment(req: ExperimentRunRequest) -> TaskAccepted:
    from aqp_models.tasks.ml_tasks import run_ml_experiment
...
def run_alpha_backtest(req: AlphaBacktestRunRequest) -> TaskAccepted:
    ...
    Drives :class:`aqp_models.alpha_backtest_experiment.AlphaBacktestExperiment`
    ...
    from aqp_models.tasks.ml_tasks import run_alpha_backtest_experiment
...
def evaluate_model(req: EvaluateRequest) -> TaskAccepted:
    from aqp_models.tasks.ml_tasks import evaluate_ml_model
...
def test_model_deployment(deployment_id: str, req: DeploymentTestRequest) -> TaskAccepted:
    from aqp_models.tasks.ml_tasks import test_ml_deployment
...
def registered_models() -> dict[str, list[str]]:
    """Introspect registered Model classes in :mod:`aqp_models.models`."""
    try:
        # Ensure submodules (and thus @register decorators) run.
        import aqp_models  # noqa: F401
        import aqp_models.models  # noqa: F401
...
def validate_pipeline_recipe(req: PipelineRecipeCreateRequest) -> dict[str, Any]:
    from aqp_models.pipeline_recipes import validate_recipe
...
def pipeline_recipe_as_node(
    ...
    """
    from aqp_models.pipeline_recipes import materialise_node_spec
...
def datasets_preview(req: DatasetPreviewRequest) -> dict[str, Any]:
    ...
    handler_cfg = req.handler_cfg or {
        "class": "Alpha158",
        "module_path": "aqp_models.features.alpha158",
        "kwargs": {
...
# ---------------------------------------------------------------------------
# Recipe catalog — surfaces YAML files under aqp_models/configs/ for the UI.
# Legacy ``configs/ml`` is checked as a fallback for repos still on the
...
def list_recipes() -> list[dict[str, Any]]:
    """Enumerate `aqp_models/configs/**/*.yaml` recipes for the training UI."""
    import os
    ...
    # ``ml.py`` lives at ``aqp_models/api/routes/ml.py``; walk three
    # levels up to land on the repo root, then descend into the new
    # canonical configs dir. Fall back to the legacy location for
    # repos still on the pre-extraction layout.
    repo_root = here.parent.parent.parent.parent
    candidates = [
        repo_root / "aqp_models" / "configs",
        repo_root / "configs" / "ml",
    ]
...
def preview_flow(flow: str, req: FlowPreviewRequest) -> dict[str, Any]:
    from aqp_models.flows import run_flow
...
def preview_flow_task(flow: str, req: FlowPreviewRequest) -> TaskAccepted:
    from aqp_models.tasks.ml_tasks import preview_ml_flow
...
def run_ml_test(req: MlTestRunRequest) -> TaskAccepted:
    ...
    from aqp_models.tasks.ml_test_tasks import run_ml_test as run_ml_test_task
...
def test_single_predict(req: TestSinglePredictRequest) -> dict[str, Any]:
    ...
    """
    if not req.sync:
        from aqp_models.tasks.ml_test_tasks import predict_single
    ...
    from aqp_models.tasks.ml_test_tasks import predict_single as _task
...
def test_batch_predict(req: TestBatchPredictRequest) -> TaskAccepted:
    from aqp_models.tasks.ml_test_tasks import predict_batch
...
def test_compare(req: TestCompareRequest) -> TaskAccepted:
    ...
    from aqp_models.tasks.ml_test_tasks import compare_models
...
def test_scenario(req: TestScenarioRequest) -> dict[str, Any]:
    ...
    """
    if not req.sync:
        from aqp_models.tasks.ml_test_tasks import scenario_perturbation
    ...
    from aqp_models.tasks.ml_test_tasks import scenario_perturbation as _task
...
async def test_upload_csv(deployment_id: str, file: Any = None) -> dict[str, Any]:
    ...
    from aqp_models.tasks.ml_test_tasks import _load_alpha
