...
class AlphaExperiment(Experiment):
    ...
    def _records(self, pred: pd.Series) -> dict[str, Any]:
        ...
        try:
            ...
            from aqp_models.recorder import PortAnaRecord, SigAnaRecord, SignalRecord

            sig = SignalRecord(model=self.model, dataset=self.dataset)
            out["SignalRecord"] = sig.generate(segment=self.segment)
            if not self.records:
                self.records = [
                    {"class": "SigAnaRecord", "module_path": "aqp_models.recorder", "kwargs": {}},
                    {"class": "PortAnaRecord", "module_path": "aqp_models.recorder", "kwargs": {}},
                ]
