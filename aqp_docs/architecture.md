...
> entry point) and [aqp_docs/index.md](index.md) (the doc map).
>
...
[aqp_docs/flows.md](flows.md) for per-flow detail.
...
```mermaid
flowchart LR
    contributor[New contributor] --> human["aqp_docs/architecture.md (this file)"]
    contributor --> agent["AGENTS.md (root)"]
    human --> docsindex[aqp_docs/index.md]
    agent --> docsindex
