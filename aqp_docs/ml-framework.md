...
> Doc map: [aqp_docs/index.md](index.md) · See [aqp_docs/factor-research.md](factor-research.md) for the alphalens-style evaluation pipeline.
...
- **`AlphaBacktestExperiment`** — combined "model used as alpha"
  ...
  [aqp_docs/ml-alpha-backtest.md](ml-alpha-backtest.md).
- **Library coverage** — TF-native (`TFEstimatorModel`),
  ...
  Sklearn Stacking / AutoPipeline. See [aqp_docs/ml-libraries.md](ml-libraries.md).
- **Lightweight workbench flows** — `regression_diagnostics`,
  ...
  [aqp_docs/ml-flows.md](ml-flows.md).
- **ML preprocessors as data-pipeline nodes** —
  ...
  [aqp_docs/ml-preprocessing-pipeline.md](ml-preprocessing-pipeline.md).
- **Interactive testing workbench** — `/ml/test/{single,batch,compare,scenario,upload-csv}`
  endpoints + tabbed webui surface. See [aqp_docs/ml-testing.md](ml-testing.md).
- **Graphical builder palette** — Source / Pipeline / Split / Model
  (per-framework) / Records / Experiment / Test / Deploy sections plus
  an Interactive Workbench drawer. See [aqp_docs/ml-builder.md](ml-builder.md).
