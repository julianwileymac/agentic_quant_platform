---
title: 'ADR 001 — Static export (Vite) over SSR for the AQP client surface'
summary: 'The AQP frontend rewrite (`aqp_client/`, Vite 7 + React 19 + Tailwind 4 + shadcn/ui) is the cutover-complete operator UI. The legacy `webui/` (Next.js 15 / antd) remains in tree only as a rollback pat...'
owner: platform-team
...
- **Supersedes**: None
- **Related**: [ADR 002 — single container client](002-single-container-client.md), [`aqp_client/CUTOVER.md`](https://github.com/julianwileymac/agentic_quant_platform/blob/main/aqp_docs/aqp_client/CUTOVER.md)

## Context

The AQP frontend rewrite (`aqp_client/`, Vite 7 + React 19 + Tailwind 4 + shadcn/ui) is the cutover-complete operator UI. The legacy `webui/` (Next.js 15 / antd) remains in tree only as a rollback path. The new `aqp_client` container needs to bundle a UI build, the legacy fallback, and the FastAPI gateway into a single deployable image.
...
3. **Static export (Vite)** — `pnpm --dir aqp_client build` emitting a single `dist/` static SPA bundle.

## Decision

The canonical UI shipped in `aqp_client` is the **Vite static export** under `aqp_client/`. The Next.js legacy `webui/` is mounted as a rollback surface at `/webui` and Solara at `/legacy`, but neither is the default landing page.

Concretely:

- Stage 1 of `/build/docker/aqp_client/Dockerfile` runs `pnpm --dir aqp_client build` and copies `aqp_client/dist/` to `/app/static/`.
...
- Vite is already canonical per [`aqp_client/CUTOVER.md`](https://github.com/julianwileymac/agentic_quant_platform/blob/main/aqp_docs/aqp_client/CUTOVER.md). Picking the in-flight stack avoids reopening the cutover debate.
...
- Cutover history: [`aqp_client/CUTOVER.md`](https://github.com/julianwileymac/agentic_quant_platform/blob/main/aqp_docs/aqp_client/CUTOVER.md)
