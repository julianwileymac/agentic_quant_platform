---
title: 'PreprocessingSpec'
summary: 'Qlibs `DataHandlerLP` applies an ordered chain of `Processor` steps (rank-norm, z-score, min-max, outlier clipping, etc.) during training. At inference time the *same* chain must be re-applied — othe...'
owner: platform-team
