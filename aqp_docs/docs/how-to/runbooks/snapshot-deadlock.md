---
title: 'Runbook — dbt snapshot deadlock'
summary: '- `dbt snapshot` runs queue indefinitely. - The `dbt_snapshots` Dagster concurrency pool shows 1 slot in use but the corresponding run is `CANCELED` or `FAILED`'
owner: sre-team
