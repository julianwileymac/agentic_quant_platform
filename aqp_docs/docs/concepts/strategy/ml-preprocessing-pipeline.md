---
title: 'ML preprocessing as data-pipeline nodes'
summary: 'Before this expansion, the only way to apply an ML preprocessing recipe was to load a `Dataset` and call `Processor.fit_process` — which only works for offline `Experiment` runs. Promoting processors ...'
owner: ml-team
