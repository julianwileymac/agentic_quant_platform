---
title: 'Strategy template catalog (Phase 7 of the multi-tenant rollout)'
summary: 'Hard rule 35 in [`AGENTS.md`](https://github.com/julianwileymac/agentic_quant_platform/blob/main/AGENTS.md): "Read-only strategy templates (LEAN, community, internal references) MUST be loaded as ``resources`` rows with ``resource_type=strategy_template``. The A...'
owner: strategy-team
...
Hard rule 35 in [`AGENTS.md`](https://github.com/julianwileymac/agentic_quant_platform/blob/main/AGENTS.md): "Read-only strategy
...
- [`aqp_docs/ownership-graph.md`](../../concepts/platform/ownership-graph.md) — the
  `translated_from` / `clones` edges live in the graph projection.
- [`aqp_docs/data-mcp.md`](../../concepts/data/data-mcp.md) — the
  `data.strategies.templates.*` tools are MCP-registered like every
  other agent surface.
- [`aqp_docs/metadata-cache.md`](../../concepts/data/metadata-cache.md) — the
