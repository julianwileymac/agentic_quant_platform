---
title: 'Observability'
summary: 'AQP ships with opt-in OpenTelemetry tracing covering the full request path: FastAPI → Celery → paper session → broker SDK → Postgres → Redis. Install the `otel` extra to enable it::'
owner: sre-team
...
> Doc map: [aqp_docs/index.md](../../intro/index.md) · Progress bus reference: [aqp_docs/flows.md#cross-cutting-progress-bus](../../concepts/platform/flows.md#cross-cutting-progress-bus).
