...
> See also: [aqp_docs/index.md](index.md) for the full doc map.
...
- **`SubscriptionDataConfig`** — The data-plane routing key. Combines
  ...
  [aqp_docs/core-types.md](core-types.md).
...
- **`Instrument` joined-table inheritance** — `instruments` is the
  ...
  See [aqp_docs/erd.md](erd.md) and
...
- **Persistent host warehouse** — `C:/aqp-warehouse` on Windows,
  ...
  staging dir, and ingest audit logs. See [aqp_docs/data-catalog.md](data-catalog.md).
...
- **`BaseStrategy`** — Abstract strategy contract under
  ...
  `on_bar`, `on_signal`, etc. See [aqp_docs/backtest-engines.md](backtest-engines.md).
...
- **`IBrokerage` / `IDataQueueHandler`** — Lean-style interfaces
  ...
  [aqp_docs/paper-trading.md](paper-trading.md).
