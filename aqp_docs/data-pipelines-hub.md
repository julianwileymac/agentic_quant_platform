...
> See also:
> [aqp_docs/datasets-catalog.md](datasets-catalog.md) — the Kedro-style
> `BaseDataset` abstraction every catalog entry maps to.
> [aqp_docs/metadata-cache.md](metadata-cache.md) — the Redis prefetch
> cache that powers `<EntityPicker>` dropdowns on every form.
> [aqp_docs/data-discovery.md](data-discovery.md) — unified browser
> for ingested + uningested catalog entries.
> [aqp_docs/airbyte-builder.md](airbyte-builder.md) — graphical Airbyte
> builder + AQP Fetcher codegen.
> [aqp_docs/dagster-sandbox.md](dagster-sandbox.md) — ephemeral
