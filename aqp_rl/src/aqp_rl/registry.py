...
Mirrors :mod:`aqp_bots.registry`:

- ``aqp_rl/configs/specs/*.yaml`` is scanned on first lookup (new
  canonical location). Legacy ``configs/rl/specs/*.yaml`` is checked
  as a fallback for repositories that still hold the old layout.
...
from aqp_rl.spec import RLExperimentSpec, load_specs_from_dir
...
# Canonical: ``aqp_rl/configs/specs/`` (this file lives at
# ``aqp_rl/src/aqp_rl/registry.py``, so ../../../configs/specs walks up
# three levels: ``aqp_rl/src/aqp_rl`` -> ``aqp_rl/src`` -> ``aqp_rl``).
_DEFAULT_DIR = Path(__file__).resolve().parent.parent.parent / "configs" / "specs"
# Legacy fallback for repos still on the pre-extraction layout.
_LEGACY_DIR = Path("configs") / "rl" / "specs"
if not _DEFAULT_DIR.exists() and _LEGACY_DIR.exists():
    _DEFAULT_DIR = _LEGACY_DIR
