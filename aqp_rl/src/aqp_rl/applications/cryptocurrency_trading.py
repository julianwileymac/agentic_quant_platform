...
1. Building a data pipeline (Iceberg by default) that ingests bars for
   the crypto basket.
2. Constructing :class:`aqp_rl.envs.finrl_crypto_env.FinRLCryptoEnv`
   from the resulting numpy arrays.
3. Driving training through :class:`aqp_rl.runtime.RLRuntime`.

For users who only want bars from a single ticker the function still
falls back gracefully to
:func:`aqp_rl.applications.stock_trading.train_stock_trading` when the
