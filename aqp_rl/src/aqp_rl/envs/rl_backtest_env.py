...
canonical FinRL-X
:class:`aqp_rl.portfolio.WeightCentricPipeline` (``f_S -> f_A -> f_T -> f_R``)
...
:class:`aqp_rl.envs.rl_backtest_env.EngineThreadBridgeMixin` in Phase
...
- ``info['truncated']`` is set on any ``RiskOverlay``-flagged breach
  ...
  reward shaper in :mod:`aqp_rl.rewards.stop_properly` reads this
...
class RLBacktestEnv(BaseRLEnv):
    ...
    data_pipeline:
        :class:`aqp_rl.core.data.BaseDataPipeline` (injected by
