...
column "Module" maps to `terraform/modules/<name>`.
...
- `docker-compose.yml` base profile → `storage` + `pipeline` + `database` modules.
- `docker-compose.viz.yml` → `pipeline` (Superset / Trino / Polaris / Airbyte / Dagster).
- `docker-compose.platform.yml` → `kubernetes` + `pipeline` (DataHub / Loki / VictoriaMetrics).

Each environment composition under `terraform/environments/` selects
...
Each environment composition under `terraform/environments/*`
