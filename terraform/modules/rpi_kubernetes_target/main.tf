...
resource "kubernetes_namespace" "aqp_admin" {
  metadata {
    name = "aqp-admin"
    labels = merge(local.labels, {
      "aqp.io/scope" = "control-plane"
    })
  }
}
...
resource "kubernetes_config_map" "frontend_auth" {
  ...
  data = {
    ...
    VITE_AUTH0_REDIRECT_URI = var.ingress_host != "" ? "https://${var.ingress_host}" : ""
  }
}

resource "kubernetes_config_map" "admin_auth" {
  metadata {
    name      = "aqp-auth-config"
    namespace = kubernetes_namespace.aqp_admin.metadata[0].name
    labels    = local.labels
  }
