"""Polygon.io NBBO quotes — incremental append-only.

Endpoint: ``GET /v3/quotes/{symbol}``
Rate-limit class: ``polygon.quotes``
...
class PolygonQuotesStream(RateLimitedHttpStream):
    """Top-of-book NBBO quotes."""

    url_base = "https://api.polygon.io"
    primary_key = ["symbol", "sip_timestamp", "sequence_number"]
    cursor_field = "sip_timestamp"

    rate_limit_service = "polygon.quotes"
    ...
    def path(self, stream_slice, **_):  # type: ignore[override]
        return f"/v3/quotes/{stream_slice['key']}"
